...
def _in_mem_auth_ctx():
    """Create an in-memory context with user, user_session, and email_verifications tables."""
    conn = sqlite3.connect(':memory:')
    db = DBContext(db_url=':memory:', connection=conn, cursor=conn.cursor(), commit=conn.commit)
    ctx = MappContext(
        server_port=8000,
    ...
    # create user table #
    conn.cursor().execute(
        'CREATE TABLE user (id INTEGER PRIMARY KEY, name TEXT, email TEXT, email_verified INTEGER DEFAULT 0)'
    )

    # create user_session table #
    conn.cursor().execute(
        'CREATE TABLE user_session (id TEXT PRIMARY KEY, user_id INTEGER, created_at TEXT)'
    )

    # create email_verifications table #
    conn.cursor().execute(
        'CREATE TABLE email_verifications (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, created_at TEXT, code_hash TEXT, verified INTEGER DEFAULT 0)'
    )
...
def _create_test_user(ctx, user_id='1', name='alice', email='alice@example.com'):
    """Insert a test user directly into the DB and return a valid access token."""
    from datetime import datetime, timezone
    import mapp.auth as auth_module
    original_key = auth_module.MAPP_AUTH_SECRET_KEY
    auth_module.MAPP_AUTH_SECRET_KEY = _TEST_SECRET_KEY
    ...
    token = jwt.encode(payload, _TEST_SECRET_KEY, algorithm='HS256')

    ctx.db.cursor.execute(
        'INSERT INTO user_session (id, user_id, created_at) VALUES (?, ?, ?)',
        (jti, user_id, now.strftime('%Y-%m-%dT%H:%M:%S'))
    )
    ctx.db.commit()

    ctx.current_access_token = lambda: token

    return token, original_key


class TestLingoComFunctions(unittest.TestCase):
    """Tests for com.send_email, com.start_email_verification, and com.verify_email_address."""

    def setUp(self):
        import mapp.auth as auth_module
        self.ctx = _in_mem_auth_ctx()
        self._token, self._original_key = _create_test_user(self.ctx)
        auth_module.MAPP_AUTH_SECRET_KEY = _TEST_SECRET_KEY
        os.environ['MAPP_SMTP_MOCK'] = 'true'
        os.environ['MAPP_EMAIL_VERIFICATION_EXPIRATION'] = '3600'

    def tearDown(self):
        import mapp.auth as auth_module
        auth_module.MAPP_AUTH_SECRET_KEY = self._original_key
        self.ctx.db.connection.close()
        os.environ.pop('MAPP_SMTP_MOCK', None)
        os.environ.pop('MAPP_EMAIL_VERIFICATION_EXPIRATION', None)

    # com.send_email tests #

    def test_send_email_mock_returns_acknowledged_struct(self):
        from mapp.com import send_email
        result = send_email(self.ctx, 'test@example.com', 'Test Subject', 'Test body')
        self.assertEqual(result['type'], 'struct')
        self.assertTrue(result['value']['acknowledged'])
        self.assertIn('test@example.com', result['value']['message'])

    def test_send_email_mock_logs_message(self):
        from mapp.com import send_email
        logged = []
        self.ctx.log = lambda msg: logged.append(msg)
        send_email(self.ctx, 'user@example.com', 'Subj', 'Hello body')
        self.assertEqual(len(logged), 1)
        self.assertIn('user@example.com', logged[0])
        self.assertIn('Subj', logged[0])
        self.assertIn('Hello body', logged[0])

    # com.start_email_verification tests #

    def test_start_email_verification_returns_acknowledged_struct(self):
        from mapp.com import start_email_verification
        result = start_email_verification(self.ctx)
        self.assertEqual(result['type'], 'struct')
        self.assertTrue(result['value']['acknowledged'])

    def test_start_email_verification_creates_db_record(self):
        from mapp.com import start_email_verification
        start_email_verification(self.ctx)
        row = self.ctx.db.cursor.execute(
            'SELECT user_id, code_hash, verified FROM email_verifications WHERE user_id = ?', ('1',)
        ).fetchone()
        self.assertIsNotNone(row, 'Expected an email_verifications record to be created')
        self.assertEqual(str(row[0]), '1')
        self.assertIsNotNone(row[1])
        self.assertEqual(row[2], 0)

    def test_start_email_verification_sends_mock_email(self):
        from mapp.com import start_email_verification
        logged = []
        self.ctx.log = lambda msg: logged.append(msg)
        start_email_verification(self.ctx)
        self.assertTrue(any('alice@example.com' in m for m in logged))
        self.assertTrue(any('verification code' in m.lower() for m in logged))

    # com.verify_email_address tests #

    def test_verify_email_address_valid_code_succeeds(self):
        from mapp.com import start_email_verification, verify_email_address
        from mapp.auth import _get_password_hash
        from datetime import datetime, timezone
        ...
        result = verify_email_address(self.ctx, code)
        self.assertEqual(result['type'], 'struct')
        self.assertTrue(result['value']['acknowledged'])

    def test_verify_email_address_valid_code_updates_user(self):
        from mapp.com import verify_email_address
        from mapp.auth import _get_password_hash
        from datetime import datetime, timezone
        ...
        verify_email_address(self.ctx, code)

        row = self.ctx.db.cursor.execute('SELECT email_verified FROM user WHERE id = ?', ('1',)).fetchone()
        self.assertEqual(row[0], 1, 'Expected email_verified to be set to 1 after verification')

    def test_verify_email_address_valid_code_deletes_record(self):
        from mapp.com import verify_email_address
        from mapp.auth import _get_password_hash
        from datetime import datetime, timezone
        ...
        self.ctx.db.commit()
        record_id = self.ctx.db.cursor.execute('SELECT last_insert_rowid()').fetchone()[0]

        verify_email_address(self.ctx, code)

        row = self.ctx.db.cursor.execute(
            'SELECT id FROM email_verifications WHERE id = ?', (record_id,)
        ).fetchone()
        self.assertIsNone(row, 'Expected email_verifications record to be deleted after verification')

    def test_verify_email_address_invalid_code_raises_auth_error(self):
        from mapp.com import verify_email_address
        from mapp.auth import _get_password_hash
        from datetime import datetime, timezone

        code_hash = _get_password_hash('correct_code')
        now = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S')
        self.ctx.db.cursor.execute(
            'INSERT INTO email_verifications (user_id, created_at, code_hash, verified) VALUES (?, ?, ?, ?)',
            ('1', now, code_hash, 0)
        ...
        with self.assertRaises(AuthenticationError):
            verify_email_address(self.ctx, 'wrong_code')

    def test_verify_email_address_expired_code_raises_auth_error(self):
        from mapp.com import verify_email_address
        from mapp.auth import _get_password_hash

        os.environ['MAPP_EMAIL_VERIFICATION_EXPIRATION'] = '1'

        code = '111222'
        code_hash = _get_password_hash(code)
        # Use a timestamp far in the past to simulate expiry
        old_time = '2000-01-01T00:00:00'
        self.ctx.db.cursor.execute(
            'INSERT INTO email_verifications (user_id, created_at, code_hash, verified) VALUES (?, ?, ?, ?)',
            ('1', old_time, code_hash, 0)
        )
        self.ctx.db.commit()
