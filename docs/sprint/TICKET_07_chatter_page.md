...
Create a custom lingo page for **chatter** — posts not tied to any forum (`forum_id = -1`). The chatter page should feel distinct from the forum instance page (Ticket 06): it represents an open stream of conversation rather than a topic-organised forum.
...
1. Show a paginated stream of top-level chatter posts (`forum_id = -1` AND `reply_to = -1`)
2. Allow expanding a post to view and page through its replies
...
5. **Ordering** — Chatter should default to order from backend api
...
- Ticket 02 — `db.read`, `db.unique_counts`
