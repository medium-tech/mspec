...
class LingoPage(tkinter.Frame):
    ...
    def __init__(self, parent): 
        ...
        self._line_first_row = -1
        self._line_row_count = 0
        self._line_tags = []
    ...
    def _render_line_buffer(self):
        if False:
            line = tkinter.Label(self, text=self._line_buffer, font=TEXT)
            line.grid(row=self._line_buffer_row, column=0, padx=0)
        else:
            #line = tkinter.Text(self, font=TEXT, wrap='word', height=1 * self._line_row_count, width=100, highlightthickness=0)
            
            #line.insert(tkinter.END, self._line_buffer)
            self._line_buffer.grid(row=self._line_buffer_row, column=0, padx=0)

            

            # for n, tag in enumerate(self._line_tags):
            #     try:
            #         line.tag_add(tag['tag'], f'{tag["start"]}.0', f'{tag["end"]}.0')
            #     except tkinter.TclError as e:
            #         print(f"tag error: {e}")
            #         breakpoint()
            self._line_buffer.tag_configure('link', foreground='blue', underline=1)
            self._line_buffer.config(state=tkinter.DISABLED)

    def _open_line_buffer(self, row:int):
        if not self._line_open:
            self._line_open = True
            self._line_buffer = tkinter.Text(self, font=TEXT, wrap='word', height=10, width=100, highlightthickness=0)
            self._line_buffer_row = row
        else:
            self._line_row_count += 1
    ...
    def render_text(self, row:int, element:dict):
        ...
        # self._line_buffer += element['text']
        self._line_buffer.insert(tkinter.END, element['text'])
    ...
    def render_break(self, row:int, element:dict):
        ...
        self._open_line_buffer(row)
        self._line_buffer.insert(tkinter.END, '\n' * element['break'])
    ...
    def render_link(self, row:int, element:dict):
        ...
        self._open_line_buffer(row)
        self._line_buffer.insert(tkinter.END, display_text, ('link',))

        # start_index = len(self._line_buffer)
        # self._line_tags.append({
        #     'tag': 'link', 
        #     'start': start_index + 1, 
        #     'end': start_index + len(display_text), 
        # })

        # self._line_buffer += display_text
