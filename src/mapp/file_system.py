...
./run.sh -f mapp-tests/files/splash.png file-system ingest-start run '{"name": "splash.png", "size": 4007485, "parts": 1, "finish": true}'
...
def ingest_start(ctx: MappContext, name: str, size: int, parts: int, content_type: str = '', finish: bool = False) -> dict:
	...
	ctx.log(f'Created file {file_record.id} with status {file_record.status} - {file_record.message}')
	...
	ctx.log(f'Exiting ingest_start with - {msg}')
...
def list_files(ctx: MappContext, offset: int = 0, size: int = 50, user_id: str = '-1') -> dict:
	...
	items = [File(*row) for row in rows]

	# fetch total count #

	ctx.db.cursor.execute(
		"SELECT COUNT(*) FROM file WHERE (? = '-1' OR user_id = ?)",
		(user_id, user_id)
	)
