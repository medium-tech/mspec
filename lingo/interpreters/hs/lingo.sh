...
resolve_run_mode() {
    ...
    log_debug 'Checking environment variable LINGO_HS_RUN_MODE'
    if [[ -n "${LINGO_HS_RUN_MODE:-}" ]]; then
        log_debug "Using LINGO_HS_RUN_MODE: $LINGO_HS_RUN_MODE"
...
require_cabal() {
    log_debug 'Checking PATH for cabal'
    if ! command -v cabal >/dev/null 2>&1; then
        log_debug 'cabal not found on PATH'
...
resolve_binary_path() {
    log_debug 'Checking environment variable LINGO_HS_BIN'
    if [[ -n "${LINGO_HS_BIN:-}" ]]; then
        log_debug "Using LINGO_HS_BIN: $LINGO_HS_BIN"
        echo "$LINGO_HS_BIN"
        return
    fi
    ...
    require_cabal
    log_debug 'Resolving binary via cabal list-bin lingolib'
    (
        cd "$SCRIPT_DIR"
...
build_binary() {
    ...
    require_cabal
    log_debug 'Executing build: cabal build'
...
run_dev() {
    log_debug 'Execution mode selected: dev (cabal run)'
    require_cabal
    log_debug "Executing: cabal run lingolib -- $*"
...
run_built() {
    ...
    log_debug 'Execution mode selected: built (resolved binary)'
    log_debug "Resolved binary path: $bin_path"

    if [[ ! -x "$bin_path" ]]; then
        log_debug 'Resolved binary missing or not executable'
