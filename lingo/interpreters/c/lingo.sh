...
resolve_run_mode() {
    ...
    log_debug 'Checking environment variable LINGO_C_RUN_MODE'
    if [[ -n "${LINGO_C_RUN_MODE:-}" ]]; then
        log_debug "Using LINGO_C_RUN_MODE: $LINGO_C_RUN_MODE"
...
resolve_binary_path() {
    log_debug 'Checking environment variable LINGO_C_BIN'
    if [[ -n "${LINGO_C_BIN:-}" ]]; then
        log_debug "Using LINGO_C_BIN: $LINGO_C_BIN"
        echo "$LINGO_C_BIN"
        return
    fi
    ...
    log_debug "Using default C binary path: $DEFAULT_BIN"
    echo "$DEFAULT_BIN"
}


resolve_compiler() {
    log_debug 'Checking environment variable LINGO_C_CC'
    if [[ -n "${LINGO_C_CC:-}" ]]; then
        log_debug "Using LINGO_C_CC: $LINGO_C_CC"
        echo "$LINGO_C_CC"
        return
    fi

    log_debug 'Checking environment variable CC'
    if [[ -n "${CC:-}" ]]; then
        log_debug "Using CC fallback: $CC"
        echo "$CC"
        return
    fi

    log_debug 'Checking PATH for gcc'
    if command -v gcc >/dev/null 2>&1; then
        log_debug 'Using gcc from PATH'
        echo 'gcc'
        return
    fi

    log_debug 'Checking PATH for cc'
    if command -v cc >/dev/null 2>&1; then
        log_debug 'Using cc from PATH'
        echo 'cc'
        return
    fi

    log_debug 'No usable C compiler found'
...
resolve_libyaml_flags() {
    ...
    log_debug 'Checking environment variable LINGO_C_LIBYAML_PREFIX'
    if [[ -n "${LINGO_C_LIBYAML_PREFIX:-}" ]]; then
        flags+=("-I${LINGO_C_LIBYAML_PREFIX}/include" "-L${LINGO_C_LIBYAML_PREFIX}/lib")
        log_debug "Using libyaml prefix from LINGO_C_LIBYAML_PREFIX: $LINGO_C_LIBYAML_PREFIX"
        printf '%s\n' "${flags[@]}"
        return
    fi
    ...
    for prefix in /opt/homebrew /usr/local; do
        if [[ -f "$prefix/include/yaml.h" ]]; then
            ...
            log_debug "Detected libyaml in prefix: $prefix"
            printf '%s\n' "${flags[@]}"
            return
        fi
    done

    log_debug 'No explicit libyaml prefix detected; relying on default compiler paths'
}
...
run_dev() {
    log_debug 'Execution mode selected: dev (compile + execute)'
    local temp_bin
    temp_bin="$(mktemp "$SCRIPT_DIR/lingolib-dev-XXXXXX")"
    chmod +x "$temp_bin"

    log_debug "Compiling temporary dev binary: $temp_bin"
    compile_binary "$temp_bin"

    log_debug "Executing temporary dev binary: $temp_bin $*"
    "$temp_bin" "$@"

    rm -f "$temp_bin"
    log_debug 'Removed temporary dev binary'
}
