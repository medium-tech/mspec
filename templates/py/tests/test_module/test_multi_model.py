...
from test_module.multi_model.client import *
...
class TestMultiModel(unittest.TestCase):
    ...
    def test_multi_model_crud(self):
        ...
        created_test_multi_model = client_create_multi_model(test_ctx, test_multi_model)
        self.assertTrue(isinstance(created_test_multi_model, MultiModel))
        created_test_multi_model.validate()
        test_multi_model.id = created_test_multi_model.id

        self.assertEqual(created_test_multi_model, test_multi_model)

        # read #

        test_multi_model_read = client_read_multi_model(test_ctx, created_test_multi_model.id)
        ...
        delete_return = client_delete_multi_model(test_ctx, created_test_multi_model.id)
        self.assertIsNone(delete_return)
        self.assertRaises(NotFoundError, client_read_multi_model, test_ctx, created_test_multi_model.id)
        ...
        multi_float_result = cursor.execute(f"SELECT value FROM multi_model_multi_float WHERE multi_model_id=? ORDER BY position", (created_test_multi_model.id,))
        self.assertEqual(len(multi_float_result.fetchall()), 0)

        multi_string_result = cursor.execute(f"SELECT value FROM multi_model_multi_string WHERE multi_model_id=? ORDER BY position", (created_test_multi_model.id,))
        self.assertEqual(len(multi_string_result.fetchall()), 0)

        multi_enum_result = cursor.execute(f"SELECT value FROM multi_model_multi_enum WHERE multi_model_id=? ORDER BY position", (created_test_multi_model.id,))
