...
# vars :: {"sample": "module.name.snake_case", "sample_item": "model.name.snake_case", "mongodb://127.0.0.1:27017": "db.default_url", "msample": "project.name.snake_case"}
...
def db_create_sample_item(ctx:dict, data:dict) -> str:
    ...
    """
    result = ctx['db']['client']['msample']['sample.sample_item'].insert_one(sample_item_verify(data))
...
def db_read_sample_item(ctx:dict, id:str) -> dict|None:
    ...
    """
    sample_items = ctx['db']['client']['msample']['sample.sample_item']
...
def db_update_sample_item(ctx:dict, id:str, data:dict) -> None:
    ...
    return :: None
    """
    ctx['db']['client']['msample']['sample.sample_item'].update_one({'_id': ObjectId(id)}, {'$set': sample_item_verify(data)})
...
def db_delete_sample_item(ctx:dict, id:str) -> None:
    ...
    ctx['db']['client']['msample']['sample.sample_item'].delete_one({'_id': ObjectId(id)})
...
def db_list_sample_item(ctx:dict, offset:int=0, limit:int=25) -> list[dict]:
    ...
    """
    sample_items = ctx['db']['client']['msample']['sample.sample_item'].find().skip(offset).limit(limit)
    items = []
